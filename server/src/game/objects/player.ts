...
export class PlayerBarn {
    ...
    addPlayer(socketId: string, joinMsg: net.JoinMsg) {
        ...
        if (group && group.players.length > 0) {
            team = this.teams[group.players[0].teamId - 1];
        }
        ...
        this.game.logger.log(`Player ${player.name} joined game ID ${this.game.id}`);
...
export class Player extends BaseGameObject {
    ...
    kill(params: DamageParams): void {
        ...
        if (this.game.map.factionMode && this.team!.livingPlayers.length <= 3) {
    ...
    pickupLoot(obj: Loot) {
        ...
        switch (def.type) {
            ...
            case "gun":
                ...
                break;
            case "helmet":
                if (this.hasRoleHelmet && !(this.role === "scout" || this.role === "healer" || this.role === "sniper" || this.role === "tank" || this.role === "demo" || this.role === "assault")) {
                    amountLeft = 1;
                    lootToAdd = obj.type;
                    pickupMsg.type = net.PickupMsgType.BetterItemEquipped;
                    break;
                    // if (!(this.role === "scout" || this.role === "healer" || this.role === "sniper" || this.role === "tank" || this.role === "demo" || this.role === "assault")) {
                    //     amountLeft = 1;
                    //     lootToAdd = obj.type;
                    //     pickupMsg.type = net.PickupMsgType.BetterItemEquipped;
                    //     break;
