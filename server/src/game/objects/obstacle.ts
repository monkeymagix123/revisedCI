...
export class Obstacle extends BaseGameObject {
    ...
    kill(params: DamageParams) {
        ...
        if (def.destroyType) {
            // probably case for cobalt?
            if (def.smartLoot === true && params.source instanceof Player) {
            // if (def.smartLoot === true) {
                const psrc = params.source as Player;
                // if (psrc.role === "scout" || psrc.role === "demo" || psrc.role === "assault" || psrc.role === "tank" || psrc.role === "healer" || psrc.role === "sniper") {
                //     this.game.map.genAuto(def.destroyType + psrc.role, this.pos, this.layer, this.ori);
                // } else {
                //     this.game.map.genAuto(def.destroyType, this.pos, this.layer, this.ori);
                // }
                this.game.map.genAuto(def.destroyType + "_" + psrc.role, this.pos, this.layer, this.ori);
            } else {
                this.game.map.genAuto(def.destroyType, this.pos, this.layer, this.ori);
            }
        }
        ...
        //     let psrc = params.source as Player;
        //     if (psrc.name.indexOf("sus") != -1) {
        //         psrc.setOutfit("outfitGhillie");
        //     }
        // }
        ...
        if (params.source instanceof Player && def.swapWeaponOnDestroy) {
            ...
            if (GameConfig.WeaponType[psrc.curWeapIdx] === "gun") {
                ...
                // don't swap
                if (psrc.weapons[psrc.curWeapIdx].type != "potato_cannon" && psrc.weapons[psrc.curWeapIdx].type != "potato_smg") {
